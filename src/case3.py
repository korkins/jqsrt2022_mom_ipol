...
if __name__ == "__main__":
    '''
    This script reads atmosphere parameters for Case 3, couples vertically
    inhomogeneous atmopshere with Lambertian surface and tests the result vs
    benchmark.
    '''
#------------------------------------------------------------------------------
#
...
#   Files with input data and benchmark results
    ...
    file_Jm = path_lut + 'Jm_case3.bin'
    file_Jp = path_lut + 'Jp_case3.bin'
    file_Tm = path_lut + 'Tm_case3.bin'
    file_Rp = path_lut + 'Rp_case3.bin'
    file_metadata = path_lut + 'metadata_case3.txt'
#
#------------------------------------------------------------------------------
#   Read metadata
    ...
    for ig1 in range(ng1):
        line_split = fcontent[28+ig1].split()
...
#------------------------------------------------------------------------------
#
#   Case 3: add Lambertian surface and test vs becnhamrk
    srfa = 0.1
#
    nruns = 1000
...
#
#   TOA flux: Fo = pi (from metadata_case3.txt)
    for imu0 in range(nmu0):
        Itoa[:, imu0, :] *= Fo
#
    I = Itoa[:, :, 0:nmu3:3]
    ...
    U = Itoa[:, :, 2:nmu3:3]
    P = np.sqrt(Q*Q + U*U)/I
    dat = np.loadtxt(path_bmark+'benchmark_case3.txt', skiprows=1)
    Ibmrk = dat[:, 3]
    Qbmrk = dat[:, 4]
    Ubmrk = dat[:, 5]
